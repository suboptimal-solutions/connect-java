...
class RoutingServerHandlerTest {
    private static final ConnectServerProtocolParameters PARAMETERS =
        new ConnectServerProtocolParameters(1024, 64);
    ...
    void setUpHandler(ConnectTransport transport) {
        channel.pipeline().addLast(ConnectServerPipeline.ROUTING_HANDLER, newRoutingHandler(transport));
    }
    ...
    void unaryContentTypeWithParametersStillInstallsUnaryHandler() {
        ...
        assertThat(channel.pipeline().get(ConnectServerPipeline.UNARY_POST_REQUEST_HANDLER))
            .isInstanceOf(UnaryPostRequestServerHandler.class);
    }
    ...
    void unsupportedApplicationContentTypeIsHandledByUnaryHandler() {
        ...
        assertThat(channel.pipeline().get(ConnectServerPipeline.ROUTING_HANDLER)).isNull();
        assertThat(channel.pipeline().get(ConnectServerPipeline.AGGREGATOR_HANDLER))
    ...
    void streamingContentTypeInstallsNamedStreamingHandler() {
        ...
        assertThat(channel.pipeline().get(ConnectServerPipeline.ROUTING_HANDLER)).isNull();
        assertThat(channel.pipeline().get(ConnectServerPipeline.STREAMING_HANDLER))
            .isInstanceOf(StreamingServerHandler.class);
    ...
    void unaryContentTypeInstallsNamedAggregatorAndUnaryHandlerInOrder() {
        ...
        assertThat(channel.pipeline().names())
            .containsSequence(ConnectServerPipeline.AGGREGATOR_HANDLER, ConnectServerPipeline.UNARY_POST_REQUEST_HANDLER);
    }
    ...
    void getInstallsNamedAggregatorAndUnaryGetHandler() {
        ...
        assertThat(channel.pipeline().get(ConnectServerPipeline.AGGREGATOR_HANDLER))
            .isInstanceOf(HttpObjectAggregator.class);
        assertThat(channel.pipeline().get(ConnectServerPipeline.UNARY_GET_REQUEST_HANDLER))
            .isInstanceOf(UnaryGetRequestServerHandler.class);
    ...
    void unaryMethodWithStreamingContentTypeReturns415(String contentType) {
        ...
        assertThat(request.refCnt()).isZero();
        assertThat(channel.pipeline().get(ConnectServerPipeline.STREAMING_HANDLER)).isNull();
        FullHttpResponse response = channel.readOutbound();
        try {
            HttpAssertions.assertThat(response).unsupportedMediaTypeError();
        } finally {
    ...
    private void assertNoConnectHandlerInstalled() {
        assertThat(channel.pipeline().get(ConnectServerPipeline.AGGREGATOR_HANDLER)).isNull();
        assertThat(channel.pipeline().get(ConnectServerPipeline.UNARY_GET_REQUEST_HANDLER)).isNull();
        assertThat(channel.pipeline().get(ConnectServerPipeline.UNARY_POST_REQUEST_HANDLER)).isNull();
        assertThat(channel.pipeline().get(ConnectServerPipeline.UNARY_RESPONSE_HANDLER)).isNull();
        assertThat(channel.pipeline().get(ConnectServerPipeline.STREAMING_HANDLER)).isNull();
    }

    private static RoutingServerHandler newRoutingHandler(ConnectTransport transport) {
        return new RoutingServerHandler(transport, ConnectServerProtocolConfig.builder(
    ...
    private static void setConnectHeaders(DefaultHttpRequest request, String contentType) {
        ...
        // before producing the inbound ConnectServerCallExchange these tests assert on.
