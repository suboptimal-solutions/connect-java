 ...
 * {@link #continueWith(ConnectServerCallObserver)}, or rejects it with a Connect-native error by
 * returning {@link #reject(ConnectError)}.
 *
 * <p>If the interceptor returns {@link Decision.Continue}, the pipeline guarantees that the
 * supplied {@link ConnectServerCallObserver} receives exactly one {@link ConnectServerCallObserver#onCallComplete}
 ...
 */
public interface ConnectServerInterceptor {
    /**
     ...
     *
     * <p>Return {@link #continueCall()} or {@link #continueWith(ConnectServerCallObserver)} to let the
    ...
    static Decision continueCall() {
        return new Decision.Continue(ConnectServerCallObserver.NOOP);
    }
     ...
     * {@link ConnectServerCallObserver#onCallComplete} for the lifetime of the call.
     */
    static Decision continueWith(ConnectServerCallObserver observer) {
    ...
    /**
     * Result of {@link ConnectServerInterceptor#interceptCall(ConnectCallExchange)}.
     *
     * <p>Use the static factory methods on {@link ConnectServerInterceptor} to create instances.
    ...
    sealed interface Decision {
         ...
         */
        record Continue(ConnectServerCallObserver observer) implements Decision {}
         ...
         */
        record Reject(ConnectServerCallObserver observer, ConnectError error) implements Decision {}
    }
}
